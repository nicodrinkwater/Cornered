...
public class MultiTrack {
    ...
    public MultiTrack(Selector page, TextureAtlas atlas, char[][] layout, int layoutWidth, int layoutHeight, int squareSize, Array<LevelData> courses){
    ...
    public void update(float delta){
        ...
        cameraPosition.y = cam2Pos.y - (scrHeight - 1080) / 2;
    ...
    protected void sortRange() {
        ...
        rangeX = Constants.SCREEN_WIDTH / squareSize + rangeExtra;
            ...
            for (int j = firstX; j < MathUtils.clamp(firstX + rangeX, 0, layoutWidth); j++) {
                if (layout[i][j] == 'W') {
                    ...
                    if(layout[i - 1][j] != '.' && layout[i][j -1] != '.' && layout[i -1][j -1] != '.'){
                        batch.draw(square, j * squareSize - shadowWidth - cameraPosition.x + xMovement, i * squareSize - shadowHeight - cameraPosition.y + 20, squareSize, 30);
                    }
                }
            }
        }
        ...
        for (int i = firstY; i < MathUtils.clamp(firstY + rangeY, 0, layoutHeight); i++) {
            for (int j = firstX; j < MathUtils.clamp(firstX + rangeX, 0, layoutWidth); j++) {
                if (layout[i][j] == 'W') {
                    ...
                    batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize- cameraPosition.y, squareSize/2 , squareSize/2, squareSize, squareSize, scaleX, scaleY, rotation);
                    ...
                    if(dist < 13 * squareSize){
                        ...
                        if(layout[i][j + 1] == 'W' && layout[i][j - 1] == 'W') {
                            batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize - cameraPosition.y, squareSize / 2, squareSize / 2, squareSize, squareSize, 1, 0.2f, rotation);
                        }
                    }
                }
            }
    ...
    private LevelData chooseData(int i, int j) {
        ...
        for (int k = 0; k < courses.size; k++) {
            if (j * squareSize > cameraPosition.x + page.getTrack1Pos() + k * page.getTrackGap() && j * squareSize < cameraPosition.x + page.getTrack1Pos() + (k + 1) * page.getTrackGap()) {
                return courses.get(k);
            }
        }
    ...
    private void drawWall3d(SpriteBatch batch, int i, int j) {
        ...
        if(layout[i][j + 1] == 'W' && layout[i][j-1] == 'W'){
            batch.draw(wallTop,j * squareSize - cameraPosition.x + xMovement, i * squareSize- cameraPosition.y, squareSize, squareSize);
        }
    }
    ...
    public void drawSkidMarks(SpriteBatch batch, int i, int j) {
        ...
        batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize -cameraPosition.y, squareSize, squareSize);
        batch.setColor(indColor.r, indColor.g, indColor.b, f /1.3f);
        batch.draw(flat, j * squareSize - cameraPosition.x + xMovement, i * squareSize - cameraPosition.y, squareSize, squareSize);
        ...
        if(layout[i][j] == 'L'){
            ...
            if(layout[i][j + 1] == 'F' || layout[i][j - 1] == 'F') {
                ...
                batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize - cameraPosition.y, squareSize / 4, squareSize);
                batch.draw(square, j * squareSize + 0.75f * squareSize - cameraPosition.x + xMovement, i * squareSize - cameraPosition.y, squareSize / 4, squareSize);
            } else {
                batch.setColor(0, 0, 0, 1f);
                batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize - cameraPosition.y, squareSize, squareSize / 4);
                batch.draw(square, j * squareSize - cameraPosition.x + xMovement, i * squareSize + 0.75f * squareSize - cameraPosition.y, squareSize, squareSize /4);
            }
        }
    }
